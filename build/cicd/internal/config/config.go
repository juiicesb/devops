...
const (
	...
	GitLabProjectBaseUrl = "https://github.com/juiicesb/devops"
...
func NewConfig(log *log.Logger, targetEnv Env, awsCredentials devdeploy.AwsCredentials) (*devdeploy.Config, error) {
	...
	// This is a hack only to be applied to the example services for this devops project. These permissions will enable
	// the service to delete RDS and Elastic Cache resources so keep costs low and ensure the deploy script continues
	// to work end to end.
	if cfg.ProjectName == "devops" {
		log.Printf("\t\tAdding extra permissions for devops examples")

		for idx, stmt := range cfg.AwsIamPolicy.PolicyDocument.Statement {
			if stmt.Sid == "DefaultServiceAccess" {
				stmt.Action = append(stmt.Action,
					"rds:DeleteDBCluster",
					"rds:DeleteDBInstance",
					"elasticache:DeleteCacheCluster")
				cfg.AwsIamPolicy.PolicyDocument.Statement[idx] = stmt
			}
		}
	}
